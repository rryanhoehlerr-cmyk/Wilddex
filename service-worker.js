...
self.addEventListener('push', (e) => { let d = { title: 'Wilddex', body: 'Something is stirring in the wild.', url: './' }; try { if (e.data) d = { ...d, ...e.data.json() }; } catch (_) {} e.waitUntil(self.registration.showNotification(d.title, { body: d.body, icon: 'assets/icons/icon-192.png', badge: 'assets/icons/favicon-32.png', data: { url: d.url || './' } })); });
...
self.addEventListener('sync', (e) => { if (e.tag === 'wilddex-sync') e.waitUntil(bc({ type: 'SYNC_NOW' })); });
